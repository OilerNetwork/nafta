...
async function main() {
  ...
  if (!process.env.RINKEBY_UNISWAP) {
    console.log("RINKEBY_UNISWAP not found in .env");
    process.exit();
  }
  const rinkebyUniswapAddress = process.env.RINKEBY_UNISWAP;
  ...
  if (!addresses.uniswapWrapper) {
    ...
    console.log(`\n\tUniswap LP NFT address: ${rinkebyUniswapAddress}\n`);
    ...
    const gasCosts = (deploymentMaxFeePerGas * 2564760) / 1e9;
    console.log(
      `\nDeploying NAFTA contract (2564760 gas) with ${deploymentMaxFeePerGas.toFixed(10)} gwei gas price will cost: ${gasCosts.toFixed(4)} ETH`,
    );
    ...
    const wrapper = await wrapperFactory.deploy(rinkebyUniswapAddress, {
  ...
  await handleEtherscanVerification(addresses.uniswapWrapper, [rinkebyUniswapAddress]);
}
